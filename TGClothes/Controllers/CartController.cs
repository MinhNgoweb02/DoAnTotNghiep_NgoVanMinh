    ...
    public class CartController : Controller
    {
        ...
        public ActionResult AddToCart(CartModel model)
        {
            ...
            if (cart != null)
            {
                ...
                if (list.Exists(x => x.Product.Id == model.ProductId))
                {
                    foreach (var item in list)
                    {
                        var stock = _productSizeService.GetStock(item.Product.Id);
                        if (item.Product.Id == model.ProductId )
                        {
        ...
        public ActionResult PaymentCOD(CustomerInfo model)
        {
            ...
            foreach (var item in cart)
            {
                ...
                var stock = _productSizeService.GetProductSizeByProductIdAndSizeId(item.Product.Id);
